...
Attempting to remove this role yourself, e.g. by leaving and rejoining the server, will result in punishment. Do not do this.'''
    ...
    @classmethod
    def get_info_embed(cls, member, title, strikes=None, role=None, banned=False):
        ...
        embed.add_field(name="Info", value='''
                        ...
                        **Joined**: {}
                        ...
                        '''.format(member.mention,
                                   ...
                                   member.id,
                                   '<t:{}:f>'.format(
                                       math.floor(calendar.timegm(member.joined_at.timetuple()))) if isinstance(member,
                                                                                                                discord.Member) else 'N/A',
    ...
    @classmethod
    def get_latest_messages_embed(cls, latest_messages, member):
    ...
    def get_timestamp(cls, timestamp):
        return timestamp.replace(tzinfo=tz.gettz('UTC')).astimezone(tz.gettz('America/Los_Angeles')).strftime(
            "%Y-%m-%d %I:%M %p")
